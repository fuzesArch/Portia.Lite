...
namespace Portia.Lite.Components.Main
{
   ...
   {
      public MatchSubgraphComponent() : base(
      ...
      protected override void AddInputFields()
      {
         ...
         InBoolean(nameof(MatchSubGraph.MatchTypes),
            Docs.SubGraphMatchTypes,
            MatchSubGraph.DefMatchTypes);

         InBoolean(nameof(MatchSubGraph.MatchIndices),
            Docs.SubGraphMatchIndices,
            MatchSubGraph.DefMatchIndices);

         InStrings(nameof(MatchSubGraph.NodeFilters),
            Docs.SubGraphMatchNodeFilters);

         InStrings(nameof(MatchSubGraph.NodeRules),
            Docs.SubGraphMatchNodeRules);

         InStrings(nameof(MatchSubGraph.EdgeFilters),
            Docs.SubGraphMatchEdgeFilters);

         InStrings(nameof(MatchSubGraph.EdgeRules),
         ...
         IGH_DataAccess da)
      {
         ...
         if (!da.GetItem(1,
               out GraphGoo patternGoo) ||
            patternGoo?.Value == null)
         {
            return;
         }

         MatchSubGraph task = new MatchSubGraph
         {
            PatternGraph = patternGoo.Value,
            MatchTypes = da.GetOptionalItem(2,
               MatchSubGraph.DefMatchTypes),
            MatchIndices = da.GetOptionalItem(3,
               MatchSubGraph.DefMatchIndices),
            NodeFilters = da.GetOptionalItems<string>(4)
                            .FromJson<IRule>()
                            .ToList(),
            NodeRules = da.GetOptionalItems<string>(5)
                          .FromJson<IRule>()
                          .ToList(),
            EdgeFilters = da.GetOptionalItems<string>(6)
                            .FromJson<IRule>()
                            .ToList(),
            EdgeRules = da.GetOptionalItems<string>(7)
                          .FromJson<IRule>()
                          .ToList()
         };
